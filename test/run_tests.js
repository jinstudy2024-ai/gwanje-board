...
test('2. 토큰 틀리면 전부 unauthorized', () => {
  ...
  actions.forEach(a => {
    eq(route_({ token: 'wrong', action: a, project: '재고앱', actor: '나' }).error, 'unauthorized', a + ' 틀린 토큰');
...
test('3. 클로드 claim Code.gs → ChatGPT 같은 자원 claim → locked 거부', () => {
  const a = call('claim', { project: '재고앱', resource: ' Code.gs ', actor: '클로드', memo: '로트 함수' });
  ok(a.ok === true, 'claim ok');
  eq(a.resource, 'Code.gs', '자원 공백 제거');
  eq(a.minutes, 120, '기본만료분 적용');
  const b = call('claim', { project: '재고앱', resource: 'Code.gs', actor: 'ChatGPT' });
  eq(b.ok, false, '두번째 claim 거부');
  eq(b.error, 'locked', 'error=locked');
  eq(b.by, '클로드', 'by=클로드');
  ok(!!b.until, 'until 있음');
  // 대소문자 다르면 다른 자원 / 다른 프로젝트면 다른 자원
  ok(call('claim', { project: '재고앱', resource: 'code.gs', actor: 'ChatGPT' }).ok, '대소문자 다르면 별개 자원');
  ok(call('claim', { project: '관제판', resource: 'Code.gs', actor: 'ChatGPT' }).ok, '다른 프로젝트면 별개 자원');
  // 본인 재잡기 → 연장
  const c = call('claim', { project: '재고앱', resource: 'Code.gs', actor: '클로드', minutes: 30 });
  ok(c.ok && c.renewed === true, '본인 재잡기 → renewed');
  // 잘못된 행위자/프로젝트/파라미터
  eq(call('claim', { project: '재고앱', resource: 'X', actor: '외계인' }).error, 'unknown_actor', '모르는 행위자');
  eq(call('claim', { project: '없는앱', resource: 'X', actor: '클로드' }).error, 'unknown_project', '모르는 프로젝트');
  eq(call('claim', { project: '재고앱', actor: '클로드' }).error, 'missing_param', '필수 파라미터 누락');
  eq(logRows().filter(r => r['행동'] === 'claim').length, 3, '일지 claim 3건');
});

test('4. 만료 5분 claim → 6분 뒤 status → 만료 처리', () => {
  const a = call('claim', { project: '재고앱', resource: '설치가이드.md', actor: 'ChatGPT', minutes: 5 });
  ok(a.ok, 'claim 5분');
  tick(4);
  let st = call('status', { project: '재고앱' });
  ok(st.locks.some(l => l.resource === '설치가이드.md'), '4분 뒤 아직 유효');
  tick(2);
  st = call('status', { project: '재고앱' });
  ...
  // 만료된 자원은 다시 잡을 수 있다
  ok(call('claim', { project: '재고앱', resource: '설치가이드.md', actor: '클로드' }).ok, '만료 후 재claim 성공');
});

test('5. release 후 재claim / 남의 잠금 release 거부 / 나 강제 해제', () => {
  // 클로드가 code.gs(소문자, ChatGPT 소유) 를 풀려 함 → forbidden
  const f = call('release', { project: '재고앱', resource: 'code.gs', actor: '클로드' });
  eq(f.error, 'forbidden', '남의 잠금 release 거부');
  eq(f.by, 'ChatGPT', 'by 표시');
  // ChatGPT 본인 해제
  const r = call('release', { project: '재고앱', resource: 'code.gs', actor: 'ChatGPT' });
  ok(r.ok && r.force === false, '본인 release ok, force=false');
  eq(call('release', { project: '재고앱', resource: 'code.gs', actor: 'ChatGPT' }).error, 'not_locked', '이미 풀린 것 release → not_locked');
  ok(call('claim', { project: '재고앱', resource: 'code.gs', actor: '클로드' }).ok, 'release 후 재claim 성공');
  // 나 강제 해제 (클로드의 Code.gs)
  const fr = call('release', { project: '재고앱', resource: 'Code.gs', actor: '나' });
...
test('6. heartbeat — 본인 연장 / 남의 것 거부', () => {
  const a = call('claim', { project: '관제판', resource: 'Index.html', actor: '클로드', minutes: 10 });
  ok(a.ok, 'claim 10분');
  tick(8);
  const hb = call('heartbeat', { project: '관제판', resource: 'Index.html', actor: '클로드' });
  ok(hb.ok, 'heartbeat ok');
  eq(hb.minutes, 120, '기본만료분만큼 연장');
  eq(call('heartbeat', { project: '관제판', resource: 'Index.html', actor: 'ChatGPT' }).error, 'forbidden', '남의 것 heartbeat 거부');
  eq(call('heartbeat', { project: '관제판', resource: '없는자원', actor: '클로드' }).error, 'not_locked', '없는 잠금 heartbeat');
  tick(100);
  ok(call('status', { project: '관제판' }).locks.some(l => l.resource === 'Index.html'), '연장 덕에 108분 뒤에도 유효');
  ok(logRows().some(r => r['행동'] === 'heartbeat'), '일지 heartbeat 기록');
});

test('7. 카드 추가·이동·수정 / 차례 넘기기 / 메모 → 전부 일지', () => {
  const c = call('card_add', { project: '재고앱', title: '사진 첨부 수정', actor: '헤드', assignee: '클로드', priority: '높음', resource: 'Code.gs' });
  ok(c.ok && /^C-/.test(c.cardId), 'card_add ok');
  eq(c.status, '할일', '기본 상태 할일');
  eq(call('card_add', { project: '재고앱', title: 'x', status: '이상한' }).error, 'bad_status', '잘못된 상태');
  ...
  const st = call('status', { project: '재고앱' });
  ok(st.doing.some(d => d.id === c.cardId), 'status 에 하는중 카드 노출');
  const e = call('card_edit', { cardId: c.cardId, assignee: 'ChatGPT', priority: '보통', actor: '헤드' });
  ...
  const lst = call('card_list', { project: '재고앱' });
  eq(lst.cards.find(x => x.id === c.cardId).assignee, 'ChatGPT', 'card_list 반영');

  const t = call('turn', { actor: '클로드', by: '헤드' });
  ...
  ok(call('log', { project: '재고앱', actor: '클로드', text: '오늘 여기까지' }).ok, 'log ok');
  eq(call('log', { project: '재고앱', actor: '클로드' }).error, 'missing_param', 'text 없으면 거부');

  const acts = logRows().map(r => r['행동']);
  ['card_add', 'card_move', 'card_edit', 'turn', 'note'].forEach(a => ok(acts.indexOf(a) >= 0, '일지에 ' + a));
  const ll = call('log_list', { project: '재고앱', limit: 3 });
...
test('8. 지시문 — 버전 증가, 최신만 목록, 이전 행 유지', () => {
  const a = call('guide_save', { project: '관제판', title: '협업 규칙', content: 'v1 내용', tags: '규칙', actor: '헤드' });
  ok(a.ok && a.version === 1, '첫 저장 v1');
  const b = call('guide_save', { project: '관제판', title: '협업 규칙', content: 'v2 내용', actor: '헤드' });
  eq(b.version, 2, '같은 제목 → v2');
  eq(readTable_('지시문').rows.length, 2, '이전 행 유지(2행)');
  const g = call('guide_get', { project: '관제판', title: '협업 규칙' });
  ok(g.ok && g.version === 2 && g.content === 'v2 내용', 'guide_get 최신 버전');
  eq(call('guide_list', { project: '관제판' }).guides.length, 1, 'guide_list 최신만 1개');
  eq(call('guide_get', { project: '관제판', title: '없음' }).error, 'not_found', '없는 지시문');
  ok(logRows().filter(r => r['행동'] === 'guide_add').length === 2, '일지 guide_add 2건');
});

test('8b. 리뷰 반영 — 담당 검증 · 캐시 무효화 · setup 전 doGet', () => {
  eq(call('card_add', { project: '재고앱', title: 'x', assignee: 'Claude' }).error, 'unknown_actor', 'card_add 모르는 담당 거부');
  const c = call('card_add', { project: '재고앱', title: '검증카드', assignee: '클로드' });
...
test('10. doGet JSON 경로 · 화면 경로 · rpc 동일 라우터', () => {
  ...
  const r = rpc(TOKEN, 'status', { project: '재고앱' });
  ok(r.ok && r.turn === '헤드', 'rpc → 같은 결과');
...
test('11. seedExampleData — 빈 시트에만, 재실행 시 거부', () => {
  ...
  sheet_('지시문').appendRow(['G-x', '관제판', '협업 규칙', '내 글', '', 1, new Date()]);
